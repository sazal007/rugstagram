              ...
              {order.items?.map((item, idx) => (
                <div key={item.product_id || idx} className="flex items-start gap-2 p-2 transition-shadow duration-200 border border-gray-200 rounded-lg sm:gap-3 sm:p-3 lg:gap-4 lg:p-4 bg-gray-50 hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(item.size || item.color) && (
                      <div className="flex flex-wrap gap-1 mt-1 sm:gap-2 sm:mt-2">
                        {item.size && (
                          <span className="inline-flex items-center px-1.5 py-0.5 text-xs font-medium bg-gray-100 rounded-full sm:px-2">
                            Size: {item.size}
                          </span>
                        )}
                        {item.color && (
                ...
                <div className="flex items-center justify-center py-6 text-gray-500 sm:py-8">
                  <div className="text-center">
                    ...
                    <p className="text-xs sm:text-sm">No products found for this order.</p>
                  </div>
                </div>
              )}
            </div>
